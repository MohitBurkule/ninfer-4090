...
load_gdn_input_projection(const GdnPlan& plan, const artifact::MaterializedArtifact& materialized) {
    ...
    return FusedGdnInputProjectionPayload{
        .query_key_value_z =
            materialized_weight(materialized, fused.query_key_value_z, 2 * TextConfig::key_dim + 2 * TextConfig::value_dim, 4096),
    };
}
...
void bind_nvfp4_text_layers(artifact::Binder& binder, BindingPlan& out) {
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (target.is_full_attention) {
            ...
            if (is_early_attention_input(layer)) {
                input = bind_weight(binder, prefix + "attention/query_key_gate_value",
                                    NumericFormat::BF16, {10240, 4096});
            } else {
                input = bind_nvfp4_weight(
                    binder, prefix + "attention/query_key_gate_value", 10240, 4096,
            ...
            if (is_bf16_attention_output(layer)) {
                target.attention.output = bind_weight(binder, prefix + "attention/output",
                                                      NumericFormat::BF16, {4096, TextConfig::query_size});
            } else {
                target.attention.output =
                    bind_nvfp4_weight(binder, prefix + "attention/output", 4096, TextConfig::query_size,
            ...
            if (is_bf16_gdn_output(layer)) {
                target.gdn.output =
                    bind_weight(binder, prefix + "gdn/output", NumericFormat::BF16, {4096, TextConfig::value_dim});
            } else {
                target.gdn.output =
                    bind_nvfp4_weight(binder, prefix + "gdn/output", 4096, TextConfig::value_dim,
        ...
        target.post_attention_norm = artifact::bind_device_tensor(
            binder, prefix + "post_attention_norm", NumericFormat::BF16, {4096});
        target.mlp.gate_up =
            bind_nvfp4_weight(binder, prefix + "mlp/gate_up", 24576, 4096,
                              prefix + "mlp/gate_up_projection/input_scale_divisor");
        target.mlp.down = bind_nvfp4_weight(binder, prefix + "mlp/down", 4096, 12288,
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    out.mtp.output =
        bind_mtp("mtp/layer/attention/output", NumericFormat::W8G32_F16S, {4096, TextConfig::query_size});
...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized)
    ...
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (source.is_full_attention) {
            ...
            target.key_norm   = artifact::materialized_tensor(backing, source.attention.key_norm,
                                                              NumericFormat::BF16, {256});
            target.output     = materialized_weight(backing, source.attention.output, 4096, TextConfig::query_size);
            ...
            target.output = materialized_weight(backing, source.gdn.output, 4096, TextConfig::value_dim);
            target.post_attention_norm = artifact::materialized_tensor(
                backing, source.post_attention_norm, NumericFormat::BF16, {4096});
            target.post_mixer = load_mlp(source.mlp, backing);
        }
    ...
    if (plan.features.mtp()) {
        ...
        mtp.attention.key         = row_view(mtp.attention.packed, TextConfig::query_size,
                                             TextConfig::kv_size);
        ...
        mtp.attention.value       = row_view(mtp.attention.packed,
                                             2 * TextConfig::query_size + TextConfig::kv_size,
                                             TextConfig::kv_size);
        mtp.query_norm =
        ...
        mtp.output              = artifact::materialized_weight(backing, plan.mtp.output,
                                                                NumericFormat::W8G32_F16S, 4096, TextConfig::query_size);
