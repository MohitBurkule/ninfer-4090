...
// Qwen3.5-9B: the 35B's head count over a 4096-wide hidden state.
...
void bf16_gdn_gating_proj_9_mma_split32_launch(Bf16GdnGatingTokenVariant variant, const Tensor& x,
                                                const Weight& a_weight, const Weight& b_weight,
                                                const Tensor& A_log, const Tensor& dt_bias,
                                                void* workspace, Tensor& g, Tensor& beta,
                                                cudaStream_t stream) {
    ...
    launch_bf16_prefill_mma<Bf16Gdn9Geometry, 32, 8>(variant, x, nullptr, 0.0F, nullptr, a_weight,
                                                      b_weight, A_log, dt_bias, workspace, g, beta,
                                                      stream);
}
