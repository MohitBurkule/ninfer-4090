...
void mtp_split_attn_in(const Tensor& attn_in, Tensor& q, Tensor& k, Tensor& gate, Tensor& v,
    ...
    // The packed row count follows the query head count: 24 heads for the 27B,
    // 16 for the Qwen3.5-9B, with four KV heads in both. attn_in holds
    // query + key + gate + value.
    const std::int32_t query_heads = attn_in.ne[0] == 14336 ? 24 : 16;
    const std::int32_t packed_rows = 256 * (2 * query_heads + 2 * 4);
    require_shape(attn_in, packed_rows, tokens, op, "attn_in");
    if (q.ne[0] != 256 || q.ne[1] != query_heads || q.ne[2] != tokens || q.ne[3] != 1) {
    ...
    if (gate.ne[0] != 256 || gate.ne[1] != query_heads || gate.ne[2] != tokens || gate.ne[3] != 1) {
