...
void attn_input_proj(const Tensor& x, const Weight& query_key_weight,
                     ...
                     cudaStream_t stream) {
    // 27B: 24 query heads of 256 over a 5120 hidden state. Qwen3.5-9B: 16 heads
    // over 4096. The KV block is four heads in both.
    const std::int32_t kHidden = x.ne[0];
    if (kHidden != 5120 && kHidden != 4096) {
        throw std::invalid_argument("attn_input_proj: x width must be 5120 or 4096");
    }
    const std::int32_t kQRows      = kHidden == 4096 ? 4096 : 6144;
    constexpr std::int32_t kKvRows = 1024;
    const std::int32_t cols        = x.ne[1];
    require_matrix(x, kHidden, cols, "x");
    require_matrix(q, kQRows, cols, "q");
    ...
    require_matrix(k, kKvRows, cols, "k");
    require_matrix(v, kKvRows, cols, "v");
    require_rowsplit(query_key_weight, QType::Q4G64_F16S, kQRows + kKvRows, kHidden, "query/key weight");
    require_rowsplit(gate_value_weight, QType::Q5G64_F16S, kQRows + kKvRows, kHidden, "gate/value weight");
